    ...
    (async () => {
      try {
        ...
        if (rs.status === 200) {
          ...
          setUser({
            username: data.username, email: loginData.email, phone: data?.phone || 0, avatar: '', token: data.token,
          });
          setBookingData({
            ...bookingData,
            user: {
          ...
          sessionStorage.setItem('session', JSON.stringify({
            user_id: data?.user_id || null, username: data?.username, email: loginData?.email, phone: data?.phone || 0, avatar: '', token: data?.token || null,
          }));
          router.push('/login');
        } else {
          setLoginError(true);
